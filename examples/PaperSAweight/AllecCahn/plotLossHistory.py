...
def getdata(thiscase):
    ...
    wIC = fun('weight for IC')*( 100 if thiscase.SAweight=='BRDR100' else 1)
    ...
    return Iter, error, (loss_IC, loss_GE),(wIC,wPDE)
...
def plotLoss_on_ax(ax, names=['PDE', 'IC'], legend=True, xlabel=True):
    for i, thiscase in enumerate(allcases):
        Iter, _, (loss_IC, loss_GE),_=getdata(thiscase)    
...
def plotWeight_on_ax(ax, names=['IC'], legend=True, xlabel=True):
    for i, thiscase in enumerate(allcases):
        Iter,_,_,(wIC,wPDE)=getdata(thiscase)   
        rIC = wIC/wPDE
        name =SAweightNames[thiscase.SAweight]
        if 'IC' in names:
    ...
    ax.set_xlabel('Epoch',fontsize=fontsize)
    ax.get_xaxis().set_visible(xlabel)
    ylabel=r"$\frac{\overline{w_I}}{\overline{w_R}}$"
...
if __name__ == "__main__":  
    ...
    # plt.suptitle('Allen-Cahn',x=0.5, y=0.92,fontsize=fontsize)      
