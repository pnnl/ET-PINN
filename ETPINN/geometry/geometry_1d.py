...
class Interval(Geometry):
    ...
    def boundary_normal(self, x):
        return -np.isclose(x, self.l).astype(np.float64) + np.isclose(x, self.r)

    def uniform_points(self, n, boundary=True):
        if boundary:
            return np.linspace(self.l, self.r, num=n, dtype=np.float64)[:, None]
        return np.linspace(
            self.l, self.r, num=n + 1, endpoint=False, dtype=np.float64
        )[1:, None]

    def log_uniform_points(self, n, boundary=True):
        eps = 0 if self.l > 0 else np.finfo(np.float64).eps
        l = np.log(self.l + eps)
        r = np.log(self.r + eps)
        if boundary:
            x = np.linspace(l, r, num=n, dtype=np.float64)[:, None]
        else:
            x = np.linspace(l, r, num=n + 1, endpoint=False, dtype=np.float64)[
                1:, None
            ]
        return np.exp(x) - eps
    ...
    def random_points(self, n, random="pseudo"):
        x = sample(n, 1, random)
        return (self.diam * x + self.l).astype(np.float64)

    def uniform_boundary_points(self, n):
        if n == 1:
            return np.array([[self.l]]).astype(np.float64)
        xl = np.full((n // 2, 1), self.l).astype(np.float64)
        xr = np.full((n - n // 2, 1), self.r).astype(np.float64)
        return np.vstack((xl, xr))

    def random_boundary_points(self, n, random="pseudo"):
        if n == 2:
            return np.array([[self.l], [self.r]]).astype(np.float64)
        return np.random.choice([self.l, self.r], n)[:, None].astype(np.float64)
