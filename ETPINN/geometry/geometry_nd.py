...
class Hypercube(Geometry):
    def __init__(self, xmin, xmax):
        ...
        self.xmin = np.array(xmin, dtype=np.defaultReal)
        self.xmax = np.array(xmax, dtype=np.defaultReal)
    ...
    def boundary_normal(self, x):
        _n = -np.isclose(x, self.xmin).astype(np.defaultReal) + np.isclose(
            x, self.xmax
        )
    ...
    def uniform_points(self, n, boundary=True):
        ...
        for i in range(self.dim):
            ...
            if boundary:
                xi.append(
                    np.linspace(
                        self.xmin[i], self.xmax[i], num=ni, dtype=np.defaultReal
                    )
                )
            else:
                xi.append(
...
class Hypersphere(Geometry):
    def __init__(self, center, radius):
        self.center = np.array(center, dtype=np.defaultReal)
        self.radius = radius
        super().__init__(
    ...
    def random_boundary_points(self, n, random="pseudo"):
        ...
        if random == "pseudo":
            X = np.random.normal(size=(n, self.dim)).astype(np.defaultReal)
        else:
