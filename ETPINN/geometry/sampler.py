...
def sample(n_samples, dimension, sampler="pseudo"):
    ...
    raise ValueError("f{sampler} sampler is not valid, which should be chosen among:",
                     ["pseudo", "LHS", "Halton", "Hammersley", "Sobol"]
                     )
...
def pseudo(n_samples, dimension):
    ...
    return np.random.random(size=(n_samples, dimension)).astype(np.defaultReal)


def quasirandom(n_samples, dimension, sampler):
    if sampler == "LHS":
        sampler = skopt.sampler.Lhs(
            lhs_type="centered", criterion="maximin", iterations=10
        )
        from pyDOE import lhs
        sampler.generate=lambda space, n:lhs(len(space),n)
    elif sampler == "Halton":
        ...
        if LooseVersion(skopt.__version__) < LooseVersion("0.9"):
            ...
            return np.asarray(
                sampler.generate(space, n_samples + 2)[2:], dtype=np.defaultReal
            )
    space = [(0.0, 1.0)] * dimension
    return np.asarray(sampler.generate(space, n_samples), dtype=np.defaultReal)
